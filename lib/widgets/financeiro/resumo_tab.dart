...
///
/// Agrega dados de transações para exibir cartões de resumo (receitas,
/// despesas, saldo), gráficos dos últimos 6 meses e distribuição por
/// categoria. É puramente apresentacional e utiliza `FinanceiroService`
...
class ResumoTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final categorias = FinanceiroService.despesasPorCategoria(
        transacoes, mesSelecionado.year, mesSelecionado.month);
      ...
      child: Column(children: [
        ...
        Container(
          ...
          child:
            ...
            Row(children: [
              Expanded(
                  child: _summaryCardDark('Receitas', resumoGeral['receitas']!,
                      const Color(0xFF34D399), Icons.arrow_upward_rounded)),
              const SizedBox(width: 10),
              Expanded(
                  child: _summaryCardDark('Despesas', resumoGeral['despesas']!,
                      const Color(0xFFFCA5A5), Icons.arrow_downward_rounded)),
            ]),
        ...
        Row(children: [
          Expanded(
              child: _summaryCard('Receitas', receitas, const Color(0xFF059669),
                  Icons.arrow_upward_rounded)),
          const SizedBox(width: 10),
          Expanded(
              child: _summaryCard('Despesas', despesas, const Color(0xFFEF4444),
        ...
        _card(
            ...
            SizedBox(
                height: 200,
                child: hist6.every((m) =>
                        (m['receitas'] as double) == 0 &&
                        (m['despesas'] as double) == 0)
                    ? const Center(
          ...
          _card(
              ...
              Column(
                ...
                final cat = todasCategorias.firstWhere((c) => c.nome == e.key,
                    orElse: () => const CategoriaFinanceira(
                        nome: 'Outros',
                        emoji: '💸',
                          ...
                          child: Column(
                            ...
                            ClipRRect(
                                ...
                                child: LinearProgressIndicator(
                                    ...
                                    minHeight: 6)),
                            Text(
                                '${(pct * 100).toStringAsFixed(1)}% das despesas',
                                style: const TextStyle(
  ...
  Widget _summaryCard(String label, double valor, Color cor, IconData icon) =>
      Container(
  ...
  Widget _summaryCardDark(
          String label, double valor, Color cor, IconData icon) =>
      Container(
...
class _BarChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final maxVal = dados.fold<double>(0, (m, d) {
      final r = d['receitas'] as double;
      final de = d['despesas'] as double;
    ...
    return BarChart(BarChartData(
      ...
      titlesData: FlTitlesData(
          bottomTitles: AxisTitles(
              sideTitles: SideTitles(
                  ...
                  getTitlesWidget: (v, _) => Text(
                      meses[(dados[v.toInt()]['mes'] as int) - 1],
                      style:
          ...
          leftTitles: AxisTitles(
              sideTitles: SideTitles(
                  ...
                  reservedSize: 42,
                  getTitlesWidget: (v, _) => Text(
                      v == 0 ? '0' : 'R\$${(v / 1000).toStringAsFixed(1)}k',
                      style:
      ...
      barGroups: List.generate(
          ...
          (i) => BarChartGroupData(x: i, barRods: [
                BarChartRodData(
                    toY: dados[i]['receitas'] as double,
                    color: const Color(0xFF059669),
                    width: 10,
                    borderRadius:
                        const BorderRadius.vertical(top: Radius.circular(4))),
                BarChartRodData(
                    toY: dados[i]['despesas'] as double,
...
class _RoiCardState extends State<_RoiCard> {
  ...
  @override
  Widget build(BuildContext context) {
    final totalR = widget.transacoes
        .where((t) => t.tipo == TipoTransacao.receita)
        .fold(0.0, (s, t) => s + t.valor);
    final totalD = widget.transacoes
    ...
    return Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Row(children: [
            ...
            SizedBox(width: 8),
            Text('ROI da Impressora',
                style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700))
          ]),
          const SizedBox(height: 4),
          const Text('Quanto do seu investimento você já recuperou',
              style: TextStyle(fontSize: 12, color: Colors.grey)),
          const SizedBox(height: 14),
          Row(children: [
            const Expanded(
                child: Text('Valor pago na impressora (R\$)',
                    style: TextStyle(
                        fontSize: 12,
                        fontWeight: FontWeight.w600,
                        color: Color(0xFF555577)))),
            ...
            SizedBox(
                ...
                child: TextField(
                    ...
                    onChanged: (v) => setState(
                        () => _investimento = double.tryParse(v) ?? 0))),
          ]),
          const SizedBox(height: 14),
          Row(children: [
            Expanded(
                child:
                    _stat('Total receitas', totalR, const Color(0xFF059669))),
            const SizedBox(width: 10),
            Expanded(
                child:
                    _stat('Total despesas', totalD, const Color(0xFFEF4444))),
            const SizedBox(width: 10),
            Expanded(
                child: _stat(
                    'Lucro acumulado',
  ...
  Widget _stat(String l, double v, Color c) =>
      Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(l, style: const TextStyle(fontSize: 10, color: Colors.grey)),
        const SizedBox(height: 2),
        Text('R\$ ${v.toStringAsFixed(2).replaceAll('.', ',')}',
            style:
