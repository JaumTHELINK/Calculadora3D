...
///
/// Permite ajustar preferências globais (material padrão, custos, impostos,
/// plataformas) e salvar no `PreferenciasService`. Também disponibiliza o
/// acesso ao backup do Google Drive.
...
class _ConfiguracoesScreenState extends State<ConfiguracoesScreen> {
  ...
  Future<void> _carregar() async {
    ...
    setState(() {
      ...
      _taxaCtrlList.clear();
      _taxaFixaCtrlList.clear();
      for (var pl in _plataformas) {
        _taxaCtrlList.add(TextEditingController(text: _fmt(pl.taxa)));
        _taxaFixaCtrlList.add(TextEditingController(
            text: pl.taxaFixa > 0 ? _fmt(pl.taxaFixa) : ''));
      }
      _loading = false;
    });
  }

  /// Carrega as preferências persistidas via `PreferenciasService` e
  /// popula os controllers do formulário.
  ...
  Future<void> _salvar() async {
    ...
    await PreferenciasService.salvarCustoEmbalagem(
        double.tryParse(_embalagemCtrl.text) ?? 0);
    await PreferenciasService.salvarTaxaIVA(
        double.tryParse(_ivaCtrl.text) ?? 0);
    await PreferenciasService.salvarDepreciacao(
        double.tryParse(_depreciacaoCtrl.text) ?? 0);
    for (int i = 0; i < _plataformas.length; i++) {
      _plataformas[i].taxa =
          double.tryParse(_taxaCtrlList[i].text) ?? _plataformas[i].taxa;
      _plataformas[i].taxaFixa =
          double.tryParse(_taxaFixaCtrlList[i].text) ?? 0.0;
    }
  ...
  /// Persiste as configurações informadas pelo usuário no `PreferenciasService`.
  /// Ao salvar, exibe uma confirmação e fecha a tela.
          ...
          : SingleChildScrollView(
              ...
              child: Column(children: [
                    ...
                    child: Row(children: [
                      ...
                      Expanded(
                          child: Column(
                              ...
                              children: [
                            const Text('Backup Google Drive',
                                style: TextStyle(
                                    color: Colors.white,
                                    fontWeight: FontWeight.w700,
                                    fontSize: 14)),
                            Text(
                                BackupService.isSignedIn
                                    ? 'Conectado: ${BackupService.userEmail}'
                ...
                SectionCard(
                    ...
                    title: 'Filamento padrão',
                    child: Column(children: [
                      _label('Material padrão'),
                      ...
                      _numField(_custoPorKgCtrl, 'Custo por kg (R\$)',
                          'Ex: 110', 'R\$'),
                    ])),
                const SizedBox(height: 12),
                SectionCard(
                    ...
                    child: Row(children: [
                      Expanded(
                          child: _numField(_potenciaCtrl,
                              'Potência da impressora', '200', 'W')),
                      const SizedBox(width: 12),
                      Expanded(
                          child: _numField(
                              _tarifaCtrl, 'Tarifa (R\$/kWh)', '0.75', 'R\$')),
                    ])),
                const SizedBox(height: 12),
                SectionCard(
                    icon: Icons.handyman_outlined,
                    title: 'Mão de obra & Embalagem',
                    child: Row(children: [
                      Expanded(
                          child: _numField(
                              _valorHoraCtrl, 'Valor hora (R\$)', '20', 'R\$')),
                      const SizedBox(width: 12),
                      Expanded(
                          child: _numField(
                              _embalagemCtrl, 'Embalagem (R\$)', '0', 'R\$')),
                    ])),
                const SizedBox(height: 12),
                SectionCard(
                    icon: Icons.tune_rounded,
                    title: 'Impostos & Depreciação',
                    child: Row(children: [
                      Expanded(
                          child: _numField(
                              _ivaCtrl, 'Imposto / IVA (%)', '0', '%')),
                      const SizedBox(width: 12),
                      Expanded(
                          child: _numField(_depreciacaoCtrl,
                              'Depreciação (R\$)', '0', 'R\$')),
                    ])),
                const SizedBox(height: 12),
                SectionCard(
                    icon: Icons.storefront_outlined,
                    title: 'Taxas das plataformas',
                    iconColor: const Color(0xFFEF4444),
                    child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          const Text('Taxa % e valor fixo por peça (R\$)',
                              style:
                          ...
                          ...List.generate(_plataformas.length, (i) {
                            ...
                            return Padding(
                                ...
                                child: Row(children: [
                                  ...
                                  _taxaField(_taxaFixaCtrlList[i], 'R\$',
                                      hint: '0'),
                                ]));
                          }),
                        ])),
  ...
  Widget _label(String t) => Text(t,
      style: const TextStyle(
          fontSize: 12, fontWeight: FontWeight.w600, color: Color(0xFF555577)));
  ...
  Widget _taxaField(TextEditingController ctrl, String suffix,
          {String hint = '0'}) =>
      SizedBox(
          width: 72,
          child: TextField(
  ...
  Widget _plataformaIcon(String nome) {
    final icons = {
      'Shopee': ('🛍️', const Color(0xFFEE4D2D)),
      'Mercado Livre': ('🛒', const Color(0xFFFFE600)),
      'TikTok Shop': ('🎵', const Color(0xFF010101)),
      'Revendedor': ('🤝', const Color(0xFF10B981))
    };
    ...
    return Container(
        ...
        height: 36,
        decoration: BoxDecoration(
            color: (entry?.$2 ?? Colors.grey).withOpacity(0.12),
