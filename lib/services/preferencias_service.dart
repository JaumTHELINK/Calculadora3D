...
/// Serviço simples para persistir e recuperar preferências do usuário
/// relacionadas ao cálculo (material padrão, tarifas, potência, plataformas).
///
/// Faz leitura/escrita direta no `SharedPreferences` e serializa a
/// lista de plataformas como JSON.
class PreferenciasService {
  ...
  static Future<void> salvarPlataformas(List<PlataformaConfig> plats) async {
    ...
    await prefs.setString(
        _plataformas, jsonEncode(plats.map((p) => p.toJson()).toList()));
  }
  ...
  static Future<Map<String, dynamic>> carregar() async {
    ...
    if (platsRaw != null) {
      try {
        ...
        plats = list
            .map((e) => PlataformaConfig.fromJson(e as Map<String, dynamic>))
            .toList();
      } catch (_) {}
    }
    return {
